...
|---|---|---|---|
| [01 — Foundation, Auth and Deployment](2026-08-19-data-room-01-foundation.md) | 1–6 | Monorepo, local Postgres + MinIO, Nest error contract, schema with three raw SQL indexes, email/password + optional Google auth, **both apps deployed** | `curl https://<vercel>/api/health` → `{"status":"ok"}`; a user can register, sign in, read `/auth/me` |
...
| [04 — Web Shell, Auth and Dashboard](2026-08-19-data-room-04-web-shell.md) | 18–20 | Tailwind + Radix primitives, typed API client with one silent refresh, sign-in/registration, Data Room dashboard | Session survives a hard reload; rooms list with real subtree totals |
...
Deployment is Task 3, before any feature. Cross-site cookie behaviour and bucket CORS are environment failures that cannot be reproduced locally, and finding them in the last hour is not a plan. Everything after that is ordered so each plan only depends on interfaces the previous one has already tested.
...
| §7 Deployment, cookie fix, bucket CORS | Task 3 |
| §8 Seed data | Task 29 |
