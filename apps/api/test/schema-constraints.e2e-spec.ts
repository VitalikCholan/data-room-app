...
async function makeRoom() {
  const user = await prisma.user.create({
    data: { email: `u-${randomUUID()}@t.io`, name: 'T', passwordHash: 'x' },
  })
  const roomId = randomUUID()
  const rootId = randomUUID()
  await prisma.dataRoom.create({
    data: { id: roomId, ownerId: user.id, name: 'R', rootNodeId: rootId },
  })
...
describe('schema constraints', () => {
  ...
  it('rejects two live siblings with the same name', async () => {
    ...
    await expect(
      prisma.node.create({ data: { ...base, name: 'Financials' } }),
  ...
  it('treats names as case-insensitive for conflicts', async () => {
    ...
    await expect(
      prisma.node.create({ data: { ...base, name: 'invoice.pdf' } }),
    ).rejects.toMatchObject({ code: 'P2002' })
  })
  ...
  it('allows reusing the name of a deleted sibling', async () => {
    ...
    const first = await prisma.node.create({
      data: { ...base, name: 'Deck.pdf' },
    })
    ...
    await expect(
      prisma.node.create({ data: { ...base, name: 'Deck.pdf' } }),
